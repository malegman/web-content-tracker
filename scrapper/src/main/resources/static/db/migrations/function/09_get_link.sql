CREATE OR REPLACE FUNCTION scrapper.get_link()
  RETURNS TABLE (id BIGINT, c_link VARCHAR(255), c_link_data JSONB, c_date_update TIMESTAMP WITH TIME ZONE, c_id_tg_chat_array BIGINT[]) AS $$
BEGIN
  RETURN QUERY
  SELECT l.id, l.c_link, l.c_date_update, l.c_date_update,
